...
import { DeleteClientModal } from '../Components/Modals';
import {
  ArrowLeft,
  User,
  Phone,
...
const ClientDetails = () => {
  ...
  const [client, setClient] = useState<Client | null>(location.state);

  // Editing states
  const {isEditing, setIsEditing} = useEdit();
  const [editForm, setEditForm] = useState<Client>(location.state || {});
  ...
  const handleSaveChanges = async () => {
    ...
    const validateField = (field: string, value: string): string | null => {
      ...
      switch (field) {
        case 'name':
          if (!trimmedValue) return t('client.nameRequired');
          if (trimmedValue.length < 2) return t('client.nameMinLength');
          if (trimmedValue.length > 100) return t('client.nameMaxLength');
          if (!validationPatterns.name.test(trimmedValue)) return t('client.nameInvalid');
          return null;

        case 'location':
          if (!trimmedValue) return t('client.locationRequired');
          if (trimmedValue.length < 2) return t('client.locationMinLength');
          return null;

        case 'phone':
          if (!trimmedValue) return t('client.phoneRequired');
          if (!validationPatterns.phone.test(trimmedValue)) return t('client.phoneInvalid');
          return null;

        case 'ar':
          if (!trimmedValue) return t('client.arRequired');
          if (!validationPatterns.ar.test(trimmedValue)) return t('client.arInvalid');
          return null;

        case 'nif':
          if (!trimmedValue) return t('client.nifRequired');
          if (!validationPatterns.nif.test(trimmedValue)) return t('client.nifInvalid');
          return null;

        case 'nis':
          if (!trimmedValue) return t('client.nisRequired');
          if (!validationPatterns.nis.test(trimmedValue)) return t('client.nisInvalid');
          return null;

        case 'rc':
          if (!trimmedValue) return t('client.rcRequired');
          if (!validationPatterns.rc.test(trimmedValue)) return t('client.rcInvalid');
          return null;

        case 'region':
          if (!trimmedValue) return t('client.regionRequired');
          if (!validationPatterns.region.test(trimmedValue)) return t('client.regionInvalid');
          return null;
    ...
    setLoading(true);
    try {
      handleInfo(t('messages.updating'));

      // Prepare client data with trimmed values
      const clientData = {
        cId: client.id,
  ...
  const handlePrintClientReport = () => {
    ...
    const printContent = generateClientReportHTML(client, combinedData, summaryStats);
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-slate-400 mt-4">Loading client data...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex items-center gap-4">
          <button
            onClick={() => {
             if(isEditing )
              {
                handleWarning(t('message.saveFirst'))
              }
              else {
                navigate(-1)
              } 
      ...
      <div className="bg-slate-900 p-6 rounded-xl shadow-md border border-slate-700">
        <div className="flex items-center justify-between mb-6">
          ...
          >
            ...
            />
            {showPaymentSection ? 'Hide' : 'Show'} Details
          </button>
        </div>
              ...
              {combinedData.length > 0 ? (
                                ...
                                <td className="px-6 py-4">
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex-1">
                                      ...
                                      >
                                        {item.docType === 'BL' ? 'Bon de Livraison' : 'Invoice'}
                                      </motion.div>
                                      {isCancelled && (
                                        <motion.div
                                          initial={{ opacity: 0, y: -10 }}
                ...
                >
                  ...
                  >
                    ...
                    >
                      <p className="text-slate-300 font-medium">{t('noDocuments')}</p>
                      <p className="text-slate-400 text-sm mt-1">
                        {t('noDocumentsFiltered', { year: yearFilter })}
                      </p>
                    </motion.div>
                  </motion.div>
                </motion.div>
...
const generateClientReportHTML = (client: Client, documents: Document[], stats: ClientReportStats) => {
      ...
      let rows = `
        <tr class="main-doc-row ${mainRowClass}">
          <td class="doc-number-cell" rowspan="${paymentRows + 1}">
            ...
            <div class="doc-type">${doc.doc_type == 'Invoice'? 'Facture': 'BL'}</div>
          </td>
      ...
      <div class="container">
        ...
        <div class="client-info">
          <div class="info-section">
            ...
            <p><strong>Adresse:</strong> ${client.location || 'Non spécifiée'}</p>
            <p><strong>Téléphone:</strong> ${client.phone || 'Non spécifié'}</p>
            <p><strong>RC:</strong> ${client.rc || 'Non spécifié'}</p>
          </div>
          <div class="info-section">
            <h3>Détails d'Enregistrement</h3>
            <p><strong>AR:</strong> ${client.ar || 'Non spécifié'}</p>
            <p><strong>NIF:</strong> ${client.nif || 'Non spécifié'}</p>
            <p><strong>NIS:</strong> ${client.nis || 'Non spécifié'}</p>
          </div>
        </div>
        ...
        <div class="footer-info">
          ...
          Pour toute question concernant ce rapport, veuillez contacter<br>
